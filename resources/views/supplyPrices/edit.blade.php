@extends("layouts.app")
@section("title", __("titles.edit_supplyPrice"))

@section("content")
	<form action="{{route('supplyPrice.update',$supplyPrice)}}" method="post">
		@csrf
		@method('PUT')
            ...
            <div class="col-sm-6">
                <select name="supply" id="supply" class="form-control">
                    @foreach($supplies as $supply)
                        <option
                            @selected(old('supply',$supplyPrice->supply->id) == $supply->id)
        ...
        <div class="form-group row">
            ...
            <div class="col-sm-6">
                <input class="form-control @error("price") is-invalid @enderror" type="number" id="price" name="price"
                       required minlength="2" value="{{ old("price", $supplyPrice->price) }}">
                @error("price")
        ...
        <div class="form-group row">
            ...
            <div class="col-sm-6">
                <input class="form-control @error("start_date") is-invalid @enderror" type="date" id="start_date" name="start_date"
                       required minlength="2" value="{{ old("start_date",$supplyPrice->start_date) }}">
                @error("start_date")
                <div class="invalid-feedback">
                    {{ $message }}
                </div>
